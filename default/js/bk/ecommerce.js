...
$(document).ready(function () {
	...
	$("#select-dealerlist").change(function(){
        ...
        var urladd = siteRoot + "/DealerLocator/Services/GetDealerList.aspx";
        var data = { 'method': 'GetDealerList', 'Regions':$(this).val()};
    ...
    $("body").on("click", '.ajaxproductlink', function(e) {
        ...
        if (zoneId != null)
            data = { 'method': 'SearchProducts', 'position': obj.attr('data-id'), 'template': 'ProductsSpecical.xslt', 'zoneid': zoneId };
    ...
    if ($(".tv-sale").length > 0) {
        if ($(".tv-sale .nav-tabs .nav-link.active").length > 0 && $(".tv-sale .nav-tabs .nav-link.active").data("id") == '-1') {
            ...
            if (zoneId != null)
                data = { 'method': 'ZoneProducts', 'position': '-1', 'template': 'ZoneProduct.xslt', 'zoneid': zoneId, 'mid': 246 };
    ...
    /*if($(".tv-onair").length > 0)
    {
        if($(".tv-onair .nav-tabs .nav-link.active").length > 0 && $(".tv-onair .nav-tabs .nav-link.active").data("id") != '-1')
        {
        console.log($(".tv-onair .nav-tabs .nav-link.active").data("id"));
        var obj = $(".tv-onair .nav-tabs .nav-link.active").eq(0);
        obj.parent().parent().find('.ajaxzoneproductlink').removeClass('active');
        obj.addClass('active');
        ...
        if (zoneId != null)
            data = { 'method': 'TvOnAirProducts', 'position': '-1', 'template': 'TvProduct.xslt', 'zoneid': zoneId};
            ...
            success: function(response) {
                if (response.success) {                   
                    ...
                    $("body").on("click", '.tv-onair .ajaxzoneproductlink', function(e) {                         
                            ...
                            var urladd = siteRoot + "/Product/Services/ProductService.aspx";
                            var data = { 'method': 'ZoneProducts', 'position': '-1', 'template': 'TvProduct.xslt'};
                            if (zoneId != null)
                                data = { 'method': 'ZoneProducts', 'position': '-1', 'template': 'TvProduct.xslt', 'zoneid': _zoneId};
    ...
    $("body").on("click", '.tv-onair .ajaxzoneproductlink', function(e) {
        ...
        var data = { 'method': 'TvOnAirProducts', 'position': '-1', 'template': 'TvProduct.xslt' };
        if (_zoneId != null)
            data = { 'method': 'TvOnAirProducts', 'position': '-1', 'template': 'TvProduct.xslt', 'zoneid': _zoneId };
...
var AjaxCart = {
    ...
    addproducttocart_details: function(button) {
        ...
        data.push({ name: 'siteID', value: siteid});
        //        data.push({ name: 'colorid', value: $(button).parent().parent().find(".product-color .product-option-input").val() });
        //        data.push({ name: 'qty', value: $(button).parent().parent().find(".input-number input").val() });
		 idl_call_conversion();
        $.ajax({
            cache: false,
            url: urladd,
...
var AjaxCheckout = {
    ...
    saveorder: function(savetodb, redirect) {
        ...
        data.push({ name: 'siteID', value: $("#checkout").attr("data-siteid") });
            ...
            success: function(response) {
                if (response.success == true) {
                    ...
                    if (response.error == 'PaymentAgree') {
						$(".div-error").remove();
                        var htmlerorrcheckout = '<span class="div-error">' + response.message + '</span>'
