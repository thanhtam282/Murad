...
$(document).ready(function () {
	...
	// $('.btn-readop1').click(function () {
	 ...
	 {
		 var defaultHeight = 350; // height when "closed"
		var text = $(".tab-content .content");
		var textHeight = text[0].scrollHeight; // the real height of the element
		var button = $(".btn-readmore");

		text.css({"max-height": defaultHeight, "overflow": "hidden"});
		$(".tab-content .content").each(function(){
		
			if(this.scrollHeight <defaultHeight)
			{
				$(this).parent().find(".btn-readmore").remove();
			}
		});
		button.on("click", function(){
			var currentText=$(this).parents().find(".content");
...
$(document).ready(function () {
	...
	{
		console.log($("body .fancybox-container"));
		$('#myModal').modal(); 
	}
    LoadProvince();
    ...
    $("body").on("click", "#btnselectDealer", function () {
        writeCookie("popupdealer", "true", 60 * 15);
        if ($("#selectDealer").val() != '' && $("#selectDealer").val() != undefined && $("#selectDealer").val() != null && $("#selectDealer").val() != 'www.muradvietnam.vn')
    ...
    function writeCookie(key, value, times) {
        ...
        date.setTime(+ date + (times * 1000)); //24 * 60 * 60 * 1000 =day
...
$(document).ready(function () {
    $('.slide-landing-4 .famous-slide').slick({
        slidesToShow: 1,
        slidesToScroll: 1,
        autoplay: true,
        autoplaySpeed: 3500,
